...
task("deploy-game", "Deploys the SportsPredictionGame contract")
  ...
  .addParam("destination", "Main Chain", "openCampusSepolia")
  ...
  .setAction(async (taskArgs) => {
    ...
    const gameContractFactory = await ethers.getContractFactory("PredictArbChain")

    //console.log(networkConfig.lzEndpoint,deployParams)
    const gameContract = await gameContractFactory.deploy(networkConfig.lzEndpoint,deployParams)

    console.log(`\PredictArbChain contract deployed to ${gameContract.address} on ${network.name}`)
    ...
    const destinationGameContractFactory=await ethers.getContractFactory("PredictEduChain",destinationChainWallet)
    const destinationGameContract=await destinationGameContractFactory.deploy(destinationChainConfig.lzEndpoint)

    console.log(`\EduChain contract deployes to ${destinationGameContract.address}`)
